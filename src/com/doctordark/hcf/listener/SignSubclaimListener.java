...
public class SignSubclaimListener implements Listener
{
        ...
        if (materialData instanceof Sign) {
            ...
            if (this.isSubclaimable(attatchedBlock)) {
                ...
                if (playerFaction.equals(factionAt)) {
                    ...
                    playerFaction.broadcast(ConfigurationService.TEAMMATE_COLOUR + player.getName() + ChatColor.YELLOW + " has created a subclaim on block type " + ChatColor.LIGHT_PURPLE + attatchedBlock.getName() + ChatColor.YELLOW + " at " + ChatColor.WHITE + '[' + attatchedBlock.getX() + ", " + attatchedBlock.getZ() + ']' + ChatColor.YELLOW + " for " + (leaderChest ? "leaders" : (actualMembers.isEmpty() ? "captains" : ("members " + ChatColor.GRAY + '[' + ChatColor.DARK_GREEN + StringUtils.join((Iterable)actualMembers, ", ") + ChatColor.GRAY + ']'))));
                }
            }
        }
    }
    ...
    public void onInventoryMoveItem(final InventoryMoveItemEvent event) {
        ...
        if (holder instanceof Chest) {
            sourceBlocks = Collections.<Block>singletonList(((Chest)holder).getBlock());
        }
        else {
